...
import frontEnd.CheckmateGUI;
import frontEnd.PawnChangeGUI;
...
public class Board {
	...
	/**
	 * @return the game which this board is a part of
	 */
	public Game getGame()
	{
		return GAME;
	 ...
	 */
	public Piece movePiece(Piece p, int newRow, int newColumn) {
	...
	/**
	 * Moves p to board[newRow][newColumn] and updates the GUI accordingly
	...
	{
		if(board[row][column].isWhite() && kingChecked(bKing))
		{
		...
		else if(!board[row][column].isWhite() && kingChecked(wKing))
		{
	 ...
	 */
	public void pawnChange (int row, int col, String type) {
		...
		GAME.getGUI().updateSquare(row, col);
		detectCheck(row, col);
	}
}
